...
public class CompressedInputStream extends RebufferingInputStream implements AutoCloseable
{
    ...
    /**
     * Flag to the background task {@link Reader} if it should continue execution.
     */
    ...
    public void position(long position) throws IOException
    {
        ...
        assert position >= streamOffset + buffer.position() : String.format("stream can only read forward; next position=%d, current position=%d",
                                                                            position, (streamOffset + buffer.position()));

        if (position > streamOffset + buffer.limit())
            reBuffer(position);

        buffer.position(Ints.checkedCast(position - streamOffset));
    }
    ...
    private void reBuffer(long position) throws IOException
    {
        if (readException != null)
        {
        ...
        try
        {
            ...
            final int uncompressedChunkLength = info.parameters.chunkLength();
            if (position < 0)
                // increment the offset into the stream based on the current buffer's read count
                streamOffset += readCount;
            else
                streamOffset = position - (position % uncompressedChunkLength);

            // buffer offset is always aligned
            streamOffset &= -uncompressedChunkLength;
        }
    ...
    {
        ByteBuffer compressed = dataBuffer.take();
        if (compressed == POISON_PILL)
        {
